"""
Scraper para terremotovenezuela.app
...
re-etiqueta el campo 'fuente' como 'terremotovenezuela.app' para
...
SOURCE_LABEL = "terremotovenezuela.app"
OUTPUT_FILENAME = "terremotovenezuela.json"


def run(output_dir, limit: Optional[int] = None, only_smoke: bool = False) -> list:
    """
    Extrae registros de terremotovenezuela.app. Al redirigir a tebusco.app
    sin backend propio, delega la extracción real al scraper de
    tebusco.app y solo re-etiqueta la fuente.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / OUTPUT_FILENAME

    print(f"[terremotovenezuela] NOTA: {SOURCE_LABEL} redirige a tebusco.app (mismo backend).")
    print("[terremotovenezuela] Extrayendo datos vía tebusco.app/tebusco-portero.php …")
    ...
    print(f"[terremotovenezuela] OK: {len(records)} registros guardados en {output_file}")
    return records
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scraper de terremotovenezuela.app")
